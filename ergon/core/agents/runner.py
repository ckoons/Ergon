...
from ergon.core.database.models import Agent, AgentExecution, AgentMessage, AgentTool, AgentFile
...
class AgentRunner:
    ...
    def _setup_agent_environment(self) -> str:
        ...
        working_dir = tempfile.mkdtemp(prefix=f"ergon_{self.agent.name}_")
    ...
    async def _run_with_tools(self, input_text: str, tools: List[AgentTool]) -> str:
        ...
        if "mail" in self.agent.name.lower() or "email" in self.agent.name.lower():
            try:
                from ergon.core.agents.mail.tools import register_mail_tools
